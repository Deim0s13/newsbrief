...
> **Housekeeping (August 2026, #352):** the entire `ansible/` directory (the `kind`/`argocd`/`podman`/`newsbrief`/`caddy` roles and `recover.yml`/`status.yml` playbooks that survived the Tekton/Smee-role removal above) was removed too. It had drifted from `scripts/infra-start.sh` — different Compose flags, a call to an already-archived kind-registry setup script, a narrower ArgoCD apply scope — making `make recover` broken on a from-scratch environment; the drift was only caught by a code-health audit, not an actual outage. `make recover`/`make status` (both macOS-only) now call `scripts/infra-start.sh` and the new `scripts/infra-status.sh` directly — Ansible is no longer a project dependency. See [docs/development/KUBERNETES.md](../development/KUBERNETES.md) for the current recovery flow.

> **Housekeeping (August 2026, #356):** `scripts/infra-start.ps1`/`infra-task-install.ps1` (a Windows kind/ArgoCD installer, mentioned in the note above) were deleted. ADR-0032 already explicitly rejected running kind on Windows ("attempted and abandoned" due to WSL2 rootless Podman unreliability) in favour of the Compose+polling path — these scripts were a pre-ADR-0032 leftover, unreferenced outside a `Makefile` hint string. Windows autostart is `scripts/compose-task-install.ps1` only (`make compose-autostart-install`). Also removed: a macOS launchd installer (`scripts/com.newsbrief.plist.template`, `make autostart-install`) that started the full standalone Compose stack (api+caddy+db) on login — undocumented, and exactly the "duplicate app alongside K8s prod" pattern #325 warned about.
